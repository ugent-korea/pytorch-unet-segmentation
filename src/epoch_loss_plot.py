...
def plotloss(csvfile):
    ...
    train_loss = plt.plot(epoch_skip, tr_loss_skip, linewidth=2, ls='--', color='#0059ba', label='Train loss')
    valid_loss = plt.plot(epoch_skip, val_loss_skip, linewidth=2, ls='--', color='#47ba00', label='Validation loss')
    # Label acc graph with regular line
    train_acc = plt.plot(epoch_skip, tr_acc_skip, linewidth=2, color='#0059ba', label='Train Accuracy')
    valid_acc = plt.plot(epoch_skip, val_acc_skip, linewidth=2, color='#47ba00', label='Validation Accuracy')
